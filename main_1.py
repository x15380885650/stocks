...
test_stock_list = [
    {'code': '601099', 'end_date': datetime.strptime('2023-07-27', '%Y-%m-%d')},
]
...
class Chooser(object):
    ...
    def monitor_strategy_4(self, code, strategy, start_date_str, end_date_str, is_test=False):
        ...
        if not k_line_list:
            return
        strategy_4_ok = strategy.strategy_match_4(code, k_line_list, m_day=5, is_test=is_test)
        if strategy_4_ok:
...
if __name__ == '__main__':
    p_end_date = datetime.strptime('2023-06-21', '%Y-%m-%d')
    c = Chooser()
    c.monitor()
    ...
    # c.choose(p_end_date=p_end_date, p_code='603767')  # 2023-07-03

    # for p_day in range(0, 10):
    #     p_end_date = datetime.strptime('2023-08-22', '%Y-%m-%d') - timedelta(days=p_day)
